...
FILEPATH=/tmp/bouquetupdater-main.tar.gz
...
if [ $? -ne 0 ]; then
    echo "Failed to download bouquetupdater package!"
    cleanup
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "Failed to extract bouquetupdater package!"
    cleanup
    exit 1
fi
...
elif [ -d "$TMPPATH/bouquetupdater-main/usr/lib64/enigma2/python/Plugins/Extensions/BouquetUpdater" ]; then
...
#########################################################
#        bouquetupdater $version INSTALLED SUCCESSFULLY       #
