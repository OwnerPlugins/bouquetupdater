...
def parse_channels(html, base_url):
    ...
    for href, data_title, card_title in matches:
        ...
        # Canali italiani
            ...
            clean_name = re.sub(r'\s+Italy$', '', clean_name, flags=re.IGNORECASE)
            clean_name = re.sub(r'\s+Italy\s+', ' ', clean_name, flags=re.IGNORECASE)
...
def write_bouquet(filename, content):
    ...
    try:
        ...
        logging.info("Bouquet scritto: {} ({} bytes)".format(filepath, len(content)))
        return True
    except IOError as e:
...
def process_dlhd(url, filename):
    ...
    try:
        # Estrai base URL
        parsed = urlparse(url)
        base_url = "{}://{}".format(parsed.scheme if parsed.scheme else 'https', 
        ...
        total_channels = sum(len(ch) for ch in groups.values()) + len(xxx_channels)
        logging.info("Trovati {} canali totali ({} italiani in {} gruppi + {} XXX)".format(
            total_channels, sum(len(ch) for ch in groups.values()), len(groups), len(xxx_channels)))
